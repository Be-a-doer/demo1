...
#####17 seeds grow towards volatiles, so 17/30 = 0.5666667, 0.567 is the fraction of seedlings that grow towards volatiles
#####This fraction is how likely the seedlings will grow towards volatiles.
...
#####The range of most plausible values (of a 95 CI) is 0.3857226 to 0.7476107.
#####It excludes the fraction expected if the seedling is unable to detect, as that value was .25 which is outside the above range.
...
#####The P value of the chisquare test is P = 0.00374, which means the null hypothesis of the death of a spouse not causing health deterioration can be rejected.  
#####The probability of this rejection of the null hypotheis being wrong is 3.74%. 
...
#####The CI range, which would be the plausible range (95%) of change would be from -201.46978 to -48.31283
#####A decrease in lead in soil is consistent with the data (not an increase), and the majority of the change values are negative.
...
####20a: What is the mean difference between areas upstream and downstream? 
####what is the 95% CI of the mean difference?

mean(electric_fish_spp$speciesUpstream) - mean(electric_fish_spp$speciesDownstream)

####The mean difference between areas upstream and downstream is -1.83333.
...
#####Dominant and subordinate individuals do differ in means of giggle spectral CV. 
#####The P-value is 0.02157, which means the null hypothesis of no difference was rejected.

###30:

#####A one-sample t-test should have been used for each group instead of combining them to a two-sampled test
#####The populations are indepedent of eachother, so should be sampled separately. 
